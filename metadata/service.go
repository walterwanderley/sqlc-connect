...
func (s *Service) InputGrpc() []string {
	...
	if s.HasCustomParams() {
		...
		for _, f := range m.Fields {
			...
			res = append(res, bindToGo("req.Msg", fmt.Sprintf("%s.%s", in, attrName), attrName, f.Type, false)...)
		}
	} else {
		for i, n := range s.InputNames {
			res = append(res, bindToGo("req.Msg", n, UpperFirstCharacter(n), s.InputTypes[i], true)...)
		}
	}
...
func (s *Service) OutputGrpc() []string {
	...
	if s.HasCustomOutput() {
		res = append(res, fmt.Sprintf("return connect.NewResponse(&pb.%sResponse{%s: to%s(result)}), nil", s.Name, camelCaseProto(canonicalName(s.Output)), canonicalName(s.Output)))
		return res
	}
	if s.EmptyOutput() {
