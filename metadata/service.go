...
func (s *Service) OutputGrpc() []string {
	...
	if s.HasCustomOutput() {
		res = append(res, fmt.Sprintf("return connect.NewResponse(&pb.%sResponse{%s: to%s(result)}), nil", s.Name, converter.CamelCaseProto(canonicalName(s.Output)), canonicalName(s.Output)))
		return res
	}
	if s.EmptyOutput() {
