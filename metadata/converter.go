...
func bindToGo(src, dst, attrName, attrType string, newVar bool) []string {
	...
	case "sql.NullTime":
		...
		res = append(res, fmt.Sprintf("if v := %s.Get%s(); v != nil {", src, camelCaseProto(attrName)))
		res = append(res, "if err := v.CheckValid(); err != nil { return nil, err }")
	...
	case "time.Time":
		...
		res = append(res, fmt.Sprintf("} else { err := fmt.Errorf(\"field %s is required\")", attrName))
	...
	case "uuid.UUID":
		...
		res = append(res, fmt.Sprintf("err = fmt.Errorf(\"invalid %s: %%s\", err.Error())", attrName))
